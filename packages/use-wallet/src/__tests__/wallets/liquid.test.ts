...
import { LiquidAuthClient } from '@algorandecosystem/liquid-auth-use-wallet-client/lib/types/LiquidAuthClient'
...
// Mock the module
vi.mock('@algorandecosystem/liquid-auth-use-wallet-client', () => ({
...
describe('LiquidWallet', () => {
  ...
  describe('disconnect', () => {
    it('disconnect: should call authClient.disconnect and update store', async () => {
      ...
      expect(store.state.wallets[WalletId.LIQUID_ALGORAND_ECOSYSTEM]).toBeUndefined()
      expect(wallet.isConnected).toBe(false)
    })
  ...
  describe('resumeSession', () => {
    it('resumeSession: should call disconnect if wallet state exists', async () => {
      ...
      store = new Store<State>({
        ...DEFAULT_STATE,
        wallets: {
          [WalletId.LIQUID_ALGORAND_ECOSYSTEM]: walletState
        }
      })
